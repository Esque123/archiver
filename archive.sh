...
do
	...
	then
		...
		then
			...
			if [[ -e $i.bck ]]
			then
				read -r -p "Since you are compressing "$i", do you want to remove the existing "$i".bck file [Y/N]? " response
				if [[ $response =~ [yY(es)*] ]]
				then
					echo ""$1".bck was removed."
					cp "$i" "$i".bck
					gzip -f "$i".bck
					echo "COMPRESSED - Archive of "$i" to "$location"/ completed on $(date)." | tee -a /home/"$USER"/scripts.log
					gzip -l "$i".bck.gz >> /home/"$USER"/scripts.log
				else
